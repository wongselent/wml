...
def __set_pattern(loot_type: int, looter_name: str, social_dir: str, *args) -> None:
    __SETUP_LOOTER.dirname_pattern: str = config.join_directory(
        social_dir,
        config.MEDIA_TYPES._str[loot_type]
    )

    __SETUP_LOOTER.filename_pattern: str = config.FILENAME_PATTERN.format(
        social=config.SOCIAL_TYPES._str(config.SOCIAL_TYPES.IG),
        looter=looter_name,
        media_type=config.MEDIA_TYPES._code[loot_type],
...
def loot_hashtag(hashtag_name: str, loot_type: int = config.MEDIA_TYPES.PIC, loot_max_count: int = 1, loot_total_count: int = 100) -> Hashtag:
    __set_pattern(loot_type, "hashtag", config.SOCIAL_TYPES._dir[config.SOCIAL_TYPES.IG]["hashtag"], hashtag_name)
